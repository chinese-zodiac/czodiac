...
async function main() {
    ...
    lr.on('line', function(line) {
        ...
        (async()=>{
            ...
            while(retry){
                try{
                    let userInfo = await pool.userInfo(line);
                    let bal = userInfo.amount;
                    const isValid = bal.gte(parseEther("1"));
                    if(isValid) output.push([line,bal.toString()]);
                    lr.resume();
                    retry = false;
                } catch (err) {
    ...
    lr.on('end',async function() {    
        ...
        fs.writeFileSync('pool-bal.csv',csvData)
    });
}
