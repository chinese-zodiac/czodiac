...
async function main() {
  ...
  const ustsdSc = await ethers.getContractAt(
    "JsonNftTemplate",
    SilverDollarNfts
  );
  const gangsSc = await ethers.getContractAt("IERC721Enumerable", Gangs);
  const erc721EnumberableSc = await ethers.getContractAt(
    "EntityStoreERC721",
    EntityStoreERC721
  );
  ...
  for (let i = 0; i < (await gangsSc.totalSupply()); i++) {
    ...
    const gangUstsdCount = Number(
      (
        await erc721EnumberableSc.getStoredERC721CountFor(
          Gangs,
          i,
          SilverDollarNfts
        )
  ...
  for (let i = 0; i < winnersToPick; i++) {
    ...
    for (let j = 0; j < gangStats.length; j++) {
      if (gangStats[j].cutoff < roll) {
        winnerGangId = gangStats[j].gangId + 1;
      } else {
