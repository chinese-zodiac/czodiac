...
contract TribePoolRescueCzf is Ownable {
    ...
    mapping(address => mapping(address => bool)) public accountWrapperIsRescued;
    ...
    function rescue(IERC20 _wrapper) public {
        ...
        require(
            accountWrapperIsRescued[msg.sender][address(_wrapper)] == false,
            "TribePoolRescueCzf: Already rescued"
        );
        accountWrapperIsRescued[msg.sender][address(_wrapper)] = true;
        czf.transferFrom(
            address(_wrapper),
            msg.sender,
            _wrapper.balanceOf(msg.sender)
        );
    }
}
