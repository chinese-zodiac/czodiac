...
contract DividendPayingToken is ERC20, Ownable {
    ...
    function distributeCZUSDDividends(uint256 amount) public onlyOwner {
    ...
    function _withdrawDividendOfUser(address user) internal returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            bool success = IERC20(CZUSD).transfer(user, _withdrawableDividend);
...
contract TribeDividends is Ownable, DividendPayingToken {
    ...
    constructor(
        ...
        uint256 _minimumTokenBalanceForDividendsWad
    )
        DividendPayingToken(
            string(abi.encodePacked("TRIBE_", symbol, "_divs")),
            string(abi.encodePacked("TRIBE_", symbol, "_divs"))
        )
    {
        claimWait = 3600;
        minimumTokenBalanceForDividends = _minimumTokenBalanceForDividendsWad;
    }
    ...
    function _transfer(
        ...
        require(false, "TRIBE_X_divs: no transfer");
    }

    function withdrawDividend() public pure override {
        require(false, "TRIBE_X_divs: use claim");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "TRIBE_X_divs: 1 min to 24 hours"
        );
        require(newClaimWait != claimWait, "TRIBE_X_divs: same value");
